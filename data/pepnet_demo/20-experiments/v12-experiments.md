...
| **ctr48**             | **4.8** |   1.0   | ⏳          | ⏳          | 继续上行，找拐点                     |
| **ctr51**             | **5.1** |   1.0   | ⏳          | ⏳          | 继续上行，找拐点                     |
| **ctr54**             | **5.4** |   1.0   | ⏳          | ⏳          | 继续上行，找拐点                     |
| **ctr57**             | **5.7** |   1.0   | ⏳          | ⏳          | 继续上行，找拐点                     |
| **ctr60**             | **6.0** |   1.0   | ⏳          | ⏳          | 上限探索                         |
...
4. **ctr45 (+0.10% CTR, +0.04% CVR) 是全网唯一双方同时优于 baseline 的实验**。CTR AUC 随权重单调上升（3.0: −0.02% → 3.3: — → 3.6: +0.05% → 4.0: +0.04% → 4.5: +0.10%），未见拐点。新增 ctr48/51/54/57/60（步长 0.3）探索上限。
...
2. **完整权重扫描揭示极宽平坦区** — CTR 权重在 [2.2, 3.6] 范围内 AUC 变化 < 0.06%，说明 CTR 有 100% 样本的梯度保障，权重缩放几乎不影响预测质量。CVR 权重增加则单调递减（cvr15 → cvr20 → weight_tune），不存在甜区。

3. **ctr45 (+0.10% CTR, +0.04% CVR) 是全网唯一双方 AUC 同时正收益的实验**。CTR AUC 随 CTR 权重单调上升（3.0→4.5 持续增长），趋势清晰。虽然幅度 < 0.1%，但这是所有 loss 调优实验中唯一有正向信号的方向。
