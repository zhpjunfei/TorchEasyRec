...
| v6_ple                             | hash_bucket ⚠️（勘误: 此前标记为 vocab_list 有误） | —                           |    PLE    |   0.7875   |   0.7878   |          +0.6pp          |
| **v6_ple_d** 🆕🏆                  | **hash_bucket**                                    | **f_req_domain (CDOT)**     |  **PLE**  | **0.7904** | **0.7922** |      **+1.0pp** 🏆       |
| **v6_domain_lsp** ✅               | **hash_bucket**                                    | **lsp (CDOT)**              | PEPNet_v2 | **0.7894** | **0.7852** |        **+0.3pp**        |
| **v6_ple_lsp** ✅                  | **hash_bucket**                                    | **lsp (CDOT)**              |  **PLE**  | **0.7859** | **0.7801** |  ❌ **-0.77pp vs ple**   |
| **v6_ple_dlsp** ⚠️                 | **hash_bucket**                                    | **f_req_domain+lsp (CDOT)** |  **PLE**  | **0.7914** | **0.7831** |  ❌ **-0.47pp vs ple**   |
| **v6_domain_dlsp** ✅              | **hash_bucket**                                    | **f_req_domain+lsp (CDOT)** | PEPNet_v2 | **0.7917** | **0.7838** |        **+0.2pp**        |
| **v6_baseline_1gpu** (1×GU8IS)     | **hash_bucket**                                    | —                           | PEPNet_v2 | **0.7873** | **0.7643** | ❌ **-0.18pp** vs 2×A10  |
| v6_baseline_4gpu (旧 T_max=40000)  | —                                                  | —                           | PEPNet_v2 |     —      |   0.7589   | ❌ T_max 错，LR 轨迹不同 |
| **v6_baseline_4gpu (T_max=37000)** | —                                                  | —                           | PEPNet_v2 | **0.7900** | **0.7569** | ❌ **-0.25pp** vs 2×A10  |

### 推翻的四条结论

1. ❌ **"2 卡优于 1 卡"** — 同 config 下 1 卡 0.764 > 2 卡 0.757。T_max 假说不成立。
1. ❌ **"f_req_page 无用"** — 旧数据集结论不适用于新数据集。hash fix 贡献 **+2.6pp**。
1. ❌ **"PLE 弱于 PEPNet"** — 1 卡时 PLE 0.748 < PEPNet 0.764；2 卡时 PLE 0.788 > PEPNet 0.782-0.786。PLE 对 batch 更敏感。**25万验证集下，0.14pp 差距有统计意义（SE ≈ 0.02-0.04pp，3-7 倍标准误）**。
1. ❌ **"f_req_domain 有价值"** — 在 hash_bucket 基线上 +0.1~0.3pp，**在 25万验证集下（SE ≈ 0.02-0.04pp）确实在噪声内**。
...
| 实验                     |   GPU   |   T_max    |   最终 LR    | 衰减比例  |  CTR AUC   |  CVR AUC   |            状态            |
| :----------------------- | :-----: | :--------: | :----------: | :-------: | :--------: | :--------: | :------------------------: |
| v6_baseline_hbs（参考）  |  2×A10  |   73242    |   0.000514   |   52.3%   |   0.7883   | **0.7822** |      ✅ 已跑（参考）       |
| v6_baseline_1gpu（修正） | 1×GU8IS | **140000** | **0.000456** | **54.3%** | **0.7873** | **0.7643** | ✅ 已跑 (2026-06-06 11:32) |
| v6_baseline_4gpu（修正） | 4×V100  | **37000**  | **0.000517** | **51.7%** | **0.7900** | **0.7569** | ✅ 已跑 (2026-06-06 09:38) |

> 注：1×GU8IS 实际 1 epoch = 76000 步（数据集略大于 1.5亿），衰减比例从 52.3% 修正到 54.3%。但即使扣除这点差异，结果仍远低于 2×A10。

### 关键发现：非单调 batch 效应，2×A10 是甜区

**三个 GPU 数的 CVR 排序：2×A10 (0.7822) > 1×GU8IS (0.7643) > 4×V100 (0.7569)**，**不是单调的 batch 效应**。

| GPU                 | T_max  | 衰减  |      CVR      |  vs 2×A10   |  SE   |
| :------------------ | :----: | :---: | :-----------: | :---------: | :---: |
| 1×GU8IS (L20, 48GB) | 140000 | 54.3% |  **0.7643**   | **−0.18pp** |  4-9  |
| **2×A10 (24GB)**    | 73242  | 52.3% | **0.7822** 🏆 |      —      |   —   |
| 4×V100 (16GB)       | 37000  | 51.7% |  **0.7569**   | **−0.25pp** | 11-13 |

可能原因（已排除纯 batch 效应假说）：

1. **GPU 类型影响显著**：A10 / V100 / L20 三种架构 FP32 算力和 distributed reduction 行为不同
1. **梯度噪声与更新次数的 U 形关系**：1-GPU（噪声大，更新 76000）+ 4-GPU（噪声小，更新 19142）都不如 2-GPU（噪声中，更新 38285）
1. **数据集固定 1 epoch 约束下**，2×A10 恰好是 batch size 和更新次数的最佳平衡

**结论：eff_batch=4096 (2×A10) 是该模型/数据量/优化器配置下的经验最优**，但这是经验性选择，背后是 GPU 类型 + batch size + 更新次数三者的协同效果。

**剩余本质差异**（T_max 修正后仍存在）：

- GPU 类型：A10 vs V100 vs L20 (GU8IS)
- 梯度噪声：1-GPU > 2-GPU > 4-GPU
- 更新次数：1-GPU 76000 步 > 2-GPU 38285 步 > 4-GPU 19142 步
...
- ✅ **Domain 特征扩展有效但有限** — lsp 扩展对 PEPNet 有效（+0.3pp），但 PLE 仍更优
- ⚠️ **PLE 配 lsp+d 反而退步** — v6_ple_dlsp 0.7831 比 v6_ple 0.7878 落后 0.47pp（12 SE 强信号）
- ✅ **1 epoch 最优** — CTR 电商场景 1 epoch 后过拟合

### 当前瓶颈 → 已超越 v1c

- ✅ **PEPNet_v2（PLE+d）CVR 0.7922 已超过 v1c 0.768（+2.4pp）**——在 7 天无负采样数据集上
- hash_fix（f_req_page hash_bucket）贡献了 +2.6pp CVR，是单一最大改进
- 旧数据集上 0.742 与 v1c 0.768 的 2.6pp 差距已用"更多数据（7天无负采样）+ hash_fix"完全弥合
- v6_domain_lsp 验证（2026-06-06）= **0.7852**（CTR 0.7894）→ lsp 特征扩展对 PEPNet 有效（+0.3pp vs baseline_hbs），但仍 **-0.26pp 弱于 PLE（0.7878）** → PLE 架构 > lsp 特征
- 设计矩阵已闭合：v6_ple_lsp = 0.7801 证实 **lsp 在 PLE 下单独就是 -0.77pp**。v6_ple_d 锁定为生产候选（CVR 0.7922）
- 📊 **统计显著性**：25万验证集下，AUC 标准误 ≈ 0.02-0.04pp。PLE vs baseline_hbs 领先 +0.56pp（14-28 SE）属极强信号；PLE vs ple_dlsp 落后 0.47pp（12 SE）也极强
- ⚠️ **GPU 数对比完成（2026-06-06）**：3 个 GPU 数 T_max 对齐后实测，**CVR 排序 2×A10 (0.7822) > 1×GU8IS (0.7643) > 4×V100 (0.7569)**，**非单调 batch 效应**。**2×A10 是经验最优**，但无法拆解 GPU 类型 vs batch size vs 更新次数的独立贡献
...
| **d in lsp=0** | **+0.44pp** ✅ | +0.21pp ✅ | **f_req_domain 是架构无关稳定增益**          |
| d in lsp=1     |   +0.30pp ✅   | -0.14pp ⚠️ | d 在 lsp 存在时 PEPNet 反而变负              |

**架构 × 特征相互作用**：

- **f_req_domain（d）通用正增益**：PEPNet +0.21pp / PLE +0.44pp
- **lsp 极性反转**：PEPNet +0.30pp / PLE -0.77pp → **PLE 与 lsp 互斥**

**🏆 当前最优：v6_ple_d = CVR 0.7922**（+1.0pp vs baseline_hbs, +2.4pp vs v1c）

### 剩余待跑

无。设计矩阵完成，v6_ple_d 锁定为生产候选。
...
### T_max 修正实验（已全部完成 2026-06-06）

| Config           |  T_max | 结果                              |
| :--------------- | -----: | :-------------------------------- |
| v6_baseline_1gpu | 140000 | ✅ CVR 0.7643（-0.18pp vs 2×A10） |
| v6_baseline_4gpu |  37000 | ✅ CVR 0.7569（-0.25pp vs 2×A10） |

**结论**：三个 GPU 数 T_max 对齐后 **2×A10 (0.7822) > 1×GU8IS (0.7643) > 4×V100 (0.7569)**，非单调 batch 效应。**2×A10 是经验最优**，但 GPU 类型、batch size、更新次数三者协同决定结果。
